...
class AR:
    def __init__(self, params_distribution=None, c=0.0, sigma=1.0, **kwargs):
        ...
        self.autocov    = self.get_autocovarianza()
    ...
    def get_autocovarianza(self):
        # Formula del libro p.59
        if self.stationary is None:
            self.is_stationary()
        if not self.stationary:
            return None
    ...
    def get_unconditional_std(self):
        ...
        if self.autocov is None:
            self.get_autocovarianza()
        
        return np.sqrt(self.autocov[0])
    ...
    def autovarianzas(self, orden: int):
        autocovarianza = self.autocov
        if self.p < orden:
            for i in range(self.p,orden):
                nueva = np.sum(np.dot(self.phi[::-1],autocovarianza[i-self.p:i]))
                autocovarianza = np.append(autocovarianza,nueva)
